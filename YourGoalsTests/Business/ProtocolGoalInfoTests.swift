...
class ProtocolGoalInfoTests: StorageTestCase {
    ...
    func testWorkedTime() {
        ...
        let goal = self.testDataCreator.createGoalWithTasks(infos: [
            (name: "Task 1", prio: 1, size: 20.0, nil ,commitmentDate: referenceDate, beginTime: nil, state: nil), // 20 minutes and yet in the abstract today task
            (name: "Task 2", prio: 2, size: 40.0, nil ,commitmentDate: referenceDate, beginTime: nil, state: nil)  // 40 minutes
            ])
