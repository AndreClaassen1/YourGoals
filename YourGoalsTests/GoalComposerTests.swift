...
class GoalComposerTests: StorageTestCase {
    ...
    func testAddTaskToExistingOnes() {
        ...
        let composer = GoalComposer(manager: self.manager)
        let goal = super.testDataCreator.createGoal(name: "Test Goal")
        let _ = try! composer.create(actionableInfo: ActionableInfo(type: .task, name: "Task 1"), toGoal: goal)
        let _ = try! composer.create(actionableInfo: ActionableInfo(type: .task, name: "Task 2"), toGoal: goal)
        let _ = try! composer.create(actionableInfo: ActionableInfo(type: .task, name: "Task 3"), toGoal: goal)
