...
class AppBadgeCalculator {
    ...
    func numberOfActiveActionables(forDate date: Date, withBackburned backburned: Bool) throws -> NSNumber {
        let actionables = try self.todayActionablesDataSource.fetchActionables(forDate: date, withBackburned: backburned, andSection: nil)
